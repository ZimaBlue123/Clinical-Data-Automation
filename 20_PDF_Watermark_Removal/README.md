# 19_PDF_Watermark_Removal（PDF 干扰区定位与审计）

本模块用于在 **不物理抹除 PDF 内容** 的前提下，定位疑似水印/页眉页脚/重复干扰区域，输出 **排除框坐标**、**审计叠加 PDF** 与 **清洗后逐页文本**，供下游抽取（如 `11_PDF_to_Excel_Rule_Extract`）跳过干扰区，并保留可追溯审计材料。
...
> 若你手边仍有旧版 v1（值为 `[[x0,top,x1,bottom], ...]` 的列表），`11_PDF_to_Excel_Rule_Extract` 仍兼容；**建议统一用本模块最新输出 v2**，以便旋转页与 `mapping_audit` 一致。
...
## 与 `11_PDF_to_Excel_Rule_Extract` 联动

1. 先用本模块生成 `{stem}_boxes.json`（建议 v2）。
2. 在 `11_PDF_to_Excel_Rule_Extract` 中：
